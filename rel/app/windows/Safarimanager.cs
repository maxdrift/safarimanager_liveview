...
namespace Safarimanager;

static class SafarimanagerMain
{
    [STAThread]
    static void Main(string[] args)
    {
        ...
        if (ElixirKit.API.IsMainInstance("org.maxdrift.safarimanager"))
        {
            Application.EnableVisualStyles();
            Application.SetCompatibleTextRenderingDefault(false);
            Application.Run(new SafarimanagerApp(url, logPath));

            var code = ElixirKit.API.Stop();
            if (code == 0) { return; }
            var message = $"Safari Manager exited with exit code {code}.\r\nLogs available at: {logPath}";
...
class SafarimanagerApp : ApplicationContext
{
    private NotifyIcon notifyIcon;
    private string? url;
    private string logPath;

    public SafarimanagerApp(string url, string logPath)
    {
