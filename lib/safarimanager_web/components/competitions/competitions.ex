defmodule SMWeb.Competitions do
  @moduledoc """
  ...
  alias SMWeb.Components.Competitions.Edit
  alias SMWeb.Components.Competitions.List
  alias SMWeb.Components.Competitions.Show
  ...
  def handle_event("confirm", %{}, %{assigns: %{to_be_deleted: [id]}} = socket) do
    socket =
      case delete(id) do
        :ok -> set_alert(socket, "info", "Competition deleted successfully", @alert_duration)
        :error -> set_alert(socket, "error", "Unable to delete Competition", @alert_duration)
      end
  ...
  def handle_event("confirm", %{}, %{assigns: %{to_be_deleted: [_ | _] = ids}} = socket) do
    socket =
      case delete(ids) do
        :ok -> set_alert(socket, "info", "Competitions deleted successfully", @alert_duration)
        :error -> set_alert(socket, "error", "Unable to delete Competitions", @alert_duration)
      end
  ...
  def handle_event(
    ...
    case Competitions.update(socket.assigns.editing_entity, params) do
      {:ok, entity} ->
        ...
        socket =
          set_alert(socket, "info", ~s(Edited competition "#{entity.name}"), @alert_duration)
  ...
  def handle_params(%{"id" => id, "action" => "edit"}, _url, socket) do
    case get(id) do
      ...
      {:error, reason} ->
        ...
        socket = set_alert(socket, "error", "Unable to edit this Competition", @alert_duration)
        {:noreply, socket}
    end
  end
  ...
  def handle_params(%{"id" => id, "action" => "show"}, _url, socket) do
    case get(id) do
      ...
      {:error, reason} ->
        socket = set_alert(socket, "error", "Unable to show this Competition", @alert_duration)
