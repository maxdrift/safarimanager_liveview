defmodule SMWeb.Components.Admin.Users do
  @moduledoc """
  Users live view
  """
  ...
  alias SMWeb.Components.Admin.Users.Edit
  ...
  def handle_event("confirm", %{}, %{assigns: %{to_be_deleted: [id]}} = socket) do
    socket =
      case delete(id) do
        :ok -> put_flash(socket, :info, "User deleted successfully")
        :error -> put_flash(socket, :error, "Unable to delete User")
      end
  ...
  def handle_event("confirm", %{}, %{assigns: %{to_be_deleted: [_ | _] = ids}} = socket) do
    socket =
      case delete(ids) do
        :ok -> put_flash(socket, :info, "Users deleted successfully")
        :error -> put_flash(socket, :error, "Unable to delete Users")
      end
  ...
  def handle_event(
    ...
    case Accounts.update(socket.assigns.editing_entity, params) do
      {:ok, entity} ->
        ...
        socket =
          put_flash(socket, :info, ~s(Edited user "#{entity.first_name} #{entity.last_name}"))
  ...
  def handle_params(%{"id" => id, "action" => "edit"}, _url, socket) do
    case get(id) do
      ...
      {:error, reason} ->
        ...
        socket = put_flash(socket, :error, "Unable to edit this User")
        {:noreply, socket}
    end
  end
  ...
  def handle_params(%{"id" => id, "action" => "show"}, _url, socket) do
    case get(id) do
      ...
      {:error, reason} ->
        socket = put_flash(socket, :error, "Unable to show this User")
