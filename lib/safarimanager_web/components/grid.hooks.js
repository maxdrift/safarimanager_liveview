let GridSelection = {
  mounted() {
    this.handleEvent("smgr:select-all", ({ gridId, value }) => {
      ...
      for (const actionBtn of document.getElementsByName(`${gridId}-selection-delete`)) {
    ...
    this.handleEvent("smgr:select-some", ({ gridId }) => {
      ...
      for (const actionBtn of document.getElementsByName(`${gridId}-selection-merge`)) {
        actionBtn.disabled = checked.length < 2;
      }
