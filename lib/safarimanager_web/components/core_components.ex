defmodule SMWeb.Components.CoreComponents do
  ...
  def input(%{type: "select"} = assigns) do
    ...
    <div>
      ...
      <select
        id={@id}
        name={@name}
        class="mt-1 block w-full py-2 px-3 border rounded-md shadow-sm focus:outline-none focus:ring-zinc-500 focus:border-zinc-500 sm:text-sm"
        multiple={@multiple}
        {@rest}
      >
  ...
  def show_modal(js \\ %JS{}, id) do
    JS.show(js, to: "##{id}", transition: {"ease-out duration-200", "opacity-0", "opacity-100"})
  end
