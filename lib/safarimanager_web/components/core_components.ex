defmodule SMWeb.Components.CoreComponents do
  ...
  See the [DaisyUI documentation](https://daisyui.com/) for available components and classes.
  Where needed, Tailwind CSS utility classes are used to enhance the components.
  See the [Tailwind CSS documentation](https://tailwindcss.com) for utility classes.
  ...
  def modal(assigns) do
    ...
    >
      <div
        id={"#{@id}-bg"}
        class="bg-neutral-950/70 fixed inset-0 transition-opacity"
        aria-hidden="true"
      />
      <div
        ...
        <div class="flex min-h-full items-center justify-center">
          <div class="w-full max-w-3xl p-4 sm:p-6 lg:py-8">
            <.focus_wrap
              ...
              class="relative hidden rounded-2xl bg-base-100 p-14 text-base-content shadow-xl shadow-black/40 ring-1 ring-base-300/60 transition"
            >
              <div class="absolute top-6 right-5">
                <button
                  phx-click={JS.exec("data-cancel", to: "##{@id}")}
                  type="button"
                  class="-m-3 flex-none p-3 text-base-content/50 hover:text-base-content/80"
  ...
  def flash(assigns) do
    ...
    <div
      ...
      role="alert"
      class={[
        "alert fixed top-2 right-2 mr-2 w-80 sm:w-96 z-50",
  ...
  def simple_form(assigns) do
    ...
    <.form :let={f} for={@for} as={@as} {@rest}>
      <div class="form-control space-y-8 bg-base-100">
  ...
  # Handle tuple format {form, field_name} by converting to FormField
  def input(%{field: {%Form{} = form, field_name}} = assigns) when is_atom(field_name) or is_binary(field_name) do
    assigns
    |> assign(:field, form[field_name])
    |> input()
  end
  ...
  def error(assigns) do
    ~H"""
    <label class="label">
      <span class="label-text-alt text-error flex gap-3">
      ...
      <table class="table table-zebra w-full">
        ...
        <tbody id={@id} phx-update={match?(%Phoenix.LiveView.LiveStream{}, @rows) && "stream"}>
          <tr :for={row <- @rows} id={@row_id && @row_id.(row)} class="hover">
            ...
            >
              <span class={[i == 0 && "font-medium"]}>
                {render_slot(col, @row_item.(row))}
              </span>
            </td>
            <td :if={@action != []} class="relative">
              <div class="flex justify-end gap-3">
                <span :for={action <- @action} class="link link-hover">
